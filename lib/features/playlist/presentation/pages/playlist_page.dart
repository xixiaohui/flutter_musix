...
class PlaylistPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (playlistId != null) {
      ...
      if (pl == null) {
        return Scaffold(appBar: AppBar(title: const Text('Playlist')), body: const AppEmptyView(title: 'Playlist not found'));
      }
      return Scaffold(
        appBar: AppBar(title: Text(pl.name)),
        body: pl.songs.isEmpty
            ? const AppEmptyView(title: 'No songs')
            : ListView.builder(padding: const EdgeInsets.only(bottom: 128), itemCount: pl.songs.length,
                itemBuilder: (_, i) {
                  final s = pl.songs[i];
                  return ListTile(
                    ...
                    onTap: () => controller.playSong(pl.songs[i], allSongs: pl.songs),
                  );
                }),
      );
    }
    ...
    return Scaffold(
      ...
      body: state.isLoading
          ...
          : state.playlists.isEmpty
              ...
              : ListView.builder(padding: const EdgeInsets.all(16), itemCount: state.playlists.length,
                  itemBuilder: (_, i) {
                    ...
                    return Card(
                      child: ListTile(
                        ...
                        onTap: () {
                          if (pl.songs.isNotEmpty) controller.playList(pl.songs);
    ...
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      actions: [
        ...
        FilledButton(onPressed: () {
          if (ctrl.text.trim().isNotEmpty) { ref.read(playlistProvider.notifier).createPlaylist(ctrl.text.trim()); Navigator.pop(ctx); }
